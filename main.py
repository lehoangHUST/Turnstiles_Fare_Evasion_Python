...
try:
    ...
    efficient_pose = EfficientPose(
        folder_path=r"/home/thaivu169/Projects/Turnstiles_Fare_Evasion_Python/output/bin_imgs", model_name="RT")
    ...
    while input_cap.isOpened():
        ...
        for track in tracker.tracks:
            ...
            cv.rectangle(drawable_frame, (int(track_box[0]), int(track_box[1])), (int(track_box[2]), int(track_box[3])),
                         [0, 153, 255], 2)
            cv.putText(drawable_frame, "ID: " + str(track.track_id), (int(track_box[0]), int(track_box[1])), 0, 1,
                       (0, 0, 255), 1)
            ...
            if ((not FORWARD_FLOW and track.pre_state == 1 and track.current_state == -1) \
                ...
                cv.rectangle(drawable_frame, (int(track_box[0]), int(track_box[1])),
                             (int(track_box[2]), int(track_box[3])), [0, 255, 255], 4)
                cv.putText(drawable_frame, "Warning: backward flow", (int(track_box[0]), int(track_box[1]) + 24), 0, 1,
                ...
                label = efficient_pose.animatePose(temp_img, drawable_frame, track_box, offset,
                                                   segment_width=int(frame.shape[0] / 80),
                                                   marker_radius=int(frame.shape[0] / 80))
                cv.rectangle(drawable_frame, (int(track_box[0]), int(track_box[1])),
                             (int(track_box[2]), int(track_box[3])), [166, 116, 2], 4)
                cv.putText(drawable_frame, label, (int(track_box[0]), int(track_box[1]) + 24), 0, 1, [166, 116, 2],
