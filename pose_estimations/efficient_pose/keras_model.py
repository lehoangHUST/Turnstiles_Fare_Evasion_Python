...
class EfficientPose:
   def __init__(self, folder_path, model_name= "IV"):
      ...
      # load model
      set_learning_phase(0) # The learning phase flag is a bool tensor (0 = test, 1 = train) to be passed as input to any Keras function that uses a different behavior at train time and test time.
      self.model = load_model(os.path.join('models', 'efficient_pose', 'keras', 'EfficientPose{0}.h5'.format(self.model_variant.upper())), custom_objects={'BilinearWeights': helpers.keras_BilinearWeights, 'Swish': helpers.Swish(helpers.eswish), 'eswish': helpers.eswish, 'swish1': helpers.swish1})
   ...
   def infer(self, batch):
      ...
      """
      if self.lite:
         batch_outputs = self.model.predict(batch)
      else:
         batch_outputs = self.model.predict(batch)[-1]
